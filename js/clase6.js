...
class Persona {
    constructor( nombre, apellido, edad, direccion, nDeCuenta, clientePreferencial, gastosDeTarjeta ){
        this.nombre = nombre;
...
const persona1 = new Persona( `Juan`, `Perez`, 32, `Calle Falsa 124`, 12345678, true, 500 );
const persona2 = new Persona ( `Carolina`, `Lerner`, 28, `Calle Falsa 124`, 91011121314, false, 2.32 );
const persona3 = new Persona ( `Fernando`, `Luna`, 34, `Av. Libertador 2000`, 192837465, true, 2320 );
const persona4 = new Persona ( `Americo`, `Rodigruez`, 53, `Av. De Mayo 3000`, 1203910293, true, 1899.99 );
const persona5 = new Persona ( `Rodrigo`, `Rodriguez`, 25, `Av. Del Arquero 6000`, 120938109238, false, 15.73 );
...
for ( let i = 0; i < personas.length; i++ ){
    ...
    if ( personas[i].clientePreferencial === true ){
        console.log( `${personas[i].nombre} tiene un 5% de descuento es sus compras! Precio original: ${personas[i].gastosDeTarjeta} dolares. Precio con descuento: ${personas[i].gastosDeTarjeta * 0.95} dolares.` );
    } else {
        console.log( `${personas[i].nombre} no tiene descuento en sus compras. Debe abonar ${personas[i].gastosDeTarjeta} dolares.` );
    }
}
